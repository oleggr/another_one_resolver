...
import sys
# TODO нигде не исользуется. Рудимент. Зачем это?
import string
import threading
...
class DomainNameSearcher:
    ...
    def _print_result(self):
        '''
        Функция вывода в консоль результатов работы программы
        '''
        ...
        print('You can find all results in file report-current time and date')
    ...
    def _report_to_file(self):
        ...
        b = datetime.datetime.now().strftime('%H.%M.%S-%d.%m.%Y')
    ...
    def _report_to_file_human_readable(self):
        ...
        filename = 'report-' + b + '_human_readable.txt'
        # TODO и зачем вообще "human_readable"? В питоне все по умолчанию human readable !

        with open(filename, 'w') as f:
            counter = 0

            f.write('{:<5}{:<20}{}\n'.format('n', 'domain', 'address'))
            f.write('====|===================|===================\n')

            for key in self.report:
                counter += 1
                # TODO неудобно читать. Скобка ")" сверху на одном уровне со строкой, где "("  (ИСПРАВЛЕНО)
                f.write(
                    '{:<5}{:<20}{}\n'.format(
                        counter, 
                        key, 
